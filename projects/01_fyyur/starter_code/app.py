...
class Venue(db.Model):
    ...
    def __repr__(self):
        return f'<Venue name={self.name}, city={self.city}, state={self.state}, address={self.address}, past_shows_count={self.count_past_shows}, upcoming_shows_count={self.count_upcoming_shows}>'
...
def search_venues():
  ...
  # search for "Music" should return "The Musical Hop" and "Park Square Live Music & Coffee"
    ...
    response={
        "count": len(venues_found), 'data': list(formatted_venues)
    }
...
def search_artists():
  ...
  artists_found = Artist.query.filter(
    Artist.name.match(f'%{search_term}%')
  ).all()
...
def edit_artist_submission(artist_id):
  ...
  if edit_artist is None:
    abort(404, 'data does not exist')
...
def edit_venue_submission(venue_id):
  ...
  return render_template('forms/edit-venue.html', form=form, venue=venue_edited)
...
def create_artist_submission():
  ...
  if not form.validate_on_submit():
    error_message = 'There''s errors within the form. Please review it'
  else:
    try:
      ...
      if exists:
        error_message = f'Artist {artist_name} already registered'
      else:
        new_artist = Artist(
            name = form.name.data,
            genres = ', '.join(form.genres.data),
      ...
      logger.exception(error, exc_info=True)
      error_message = f'An error occured. Artist {artist_name} could not be created'
...
def create_show_submission():
  ...
  if not form.validate_on_submit():
    ...
    try:
      ...
      logger.exception(error, exc_info=True)
      error_message = f'An error occured while show creation. Sorry, this could not be created'
